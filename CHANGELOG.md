...
**Audio**
- **Distortion Amount now defaults to 20% instead of 0%.** Below 0.5% the distortion stage is a true bypass, so a freshly inserted instance previously passed audio through unchanged — a distortion plugin that visibly did nothing until a knob was turned. The new default (and the "Default" factory preset, which shares the same baseline) now produces an audible, moderate effect out of the box; Auto Gain (on by default) keeps the level matched.
