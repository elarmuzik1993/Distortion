...
- **EXTREME crack intensification**: toggling EXTREME crossfades an intensified crack layer in over ~0.28s (quintic ease, `Oscilloscope::kExtremeFadeSeconds`), and back out on release. Driven by the scope's existing repaint tick — no new timer, no audio-thread involvement.
