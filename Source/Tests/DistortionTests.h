...
/** Run all plugin tests and log results.
    NOTE: This function should NOT be called from PluginProcessor constructor
    as it creates additional PluginProcessor instances which causes crashes.
    Use JUCE's UnitTestRunner extra or a separate test executable instead.
*/
inline void runAllPluginTests()
{
    static std::atomic<bool> testsHaveRun{ false };

    // Only run tests once per session
    if (testsHaveRun.exchange(true))
        return;

    // Register tests before running
    registerAllTests();

    // Create a log file for test results
    juce::File logFile = juce::File::getSpecialLocation(juce::File::tempDirectory)
                            .getChildFile("DistortionTestResults.txt");
    logFile.deleteFile();

    juce::StringArray logLines;

    auto log = [&logLines](const juce::String& msg) {
        logLines.add(msg);
    ...
    log("");
    log("================================================================================");
    log("                    DISTORTION PLUGIN UNIT TESTS");
    log("================================================================================");
    log("");

    juce::UnitTestRunner runner;
    runner.setAssertOnFailure(false);   // Don't break debugger on failure
    runner.setPassesAreLogged(false);   // Only log failures to keep output clean
    ...
    for (int i = 0; i < runner.getNumResults(); ++i)
    {
        ...
        {
            log("FAILED: " + result->unitTestName + " (" + juce::String(result->passes) +
                " passed, " + juce::String(result->failures) + " failed)");
            for (const auto& message : result->messages)
                log("  - " + message);
        }
        else
        {
            log("PASSED: " + result->unitTestName + " (" + juce::String(result->passes) + " assertions)");
        }
    }
    ...
    if (totalFailures == 0)
    {
        log("  ALL TESTS PASSED: " + juce::String(totalPasses) + " assertions");
    }
    else
    {
        log("  TESTS FAILED: " + juce::String(totalPasses) + " passed, "
            + juce::String(totalFailures) + " FAILED");
    }
    ...
    // Write to file
    logFile.replaceWithText(logLines.joinIntoString("\n"));
}
