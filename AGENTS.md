...
- **CI/CD**: GitHub Actions for automated Windows/Linux/macOS release artifacts. **Windows, Linux and macOS (`macos-14`) all run on every push/PR**; only the `release` job is gated, to `v*` tags. A `concurrency` group cancels superseded non-tag runs. macOS was once tag-and-dispatch-only because those runners bill at 10× minutes (Linux 1×, Windows 2×), but that multiplier only applies to *metered* minutes and standard runners are free on public repos — the gate cost coverage and saved nothing. It matters because macOS is the only job building the universal binary (arm64 + x86_64) and running `auval`, and the platform most exposed to SDK drift (pinned JUCE 7.0.12 uses APIs newer macOS SDKs removed), so a release tag used to be the first thing to exercise that path. It is now covered before merge.
