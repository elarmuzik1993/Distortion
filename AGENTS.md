# Distortion — Source of Truth (AGENTS.md)
...
## Project Overview
**Distortion** is a professional JUCE audio plugin by Monolit Beatz featuring multi-stage distortion processing, LA2A-style optical compression, and advanced signal processing. The plugin supports VST3 and Standalone formats.
...
- **XY Morph Toggle**: An "XY Morph" pill toggle in the Settings INTERFACE section enables/disables the invisible `XYMorphPad` overlay on the oscilloscope. When off (default), scope clicks pass through and no parameter morphing occurs; when on, dragging the pad morphs 6 distortion params via `morphDistortionParameters`. State persists in `settings.xml` (`xyMorph` attribute) and is applied via `applyXYMorphEnabled` (toggles mouse interception + the pad's 30 Hz timer). Visibility stays governed by the oscilloscope toggle.
